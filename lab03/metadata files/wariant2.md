...
### 1. ZALETA: Wydajność odczytu - wszystkie dane w jednym zapytaniu

Pobieranie pełnych informacji o firmie wraz z jej wycieczkami
```js
db.companies.findOne({ _id: company1Id });
```

Pobieranie szczegółowej listy rezerwacji użytkownika wraz z danymi wycieczek
```js
db.users.findOne(
  { _id: user2Id },
...
### 2. ZALETA: Łatwe wyszukiwanie powiązanych danych bez łączenia kolekcji

Znalezienie wszystkich firm oferujących wycieczki do określonej destynacji
```js
db.companies.find(
...
Znalezienie użytkowników z rezerwacjami na konkretną wycieczkę
```js
db.users.find(
...
### 3. ZALETA: Atomowe aktualizacje powiązanych danych

Aktualizacja liczby rezerwacji dla wycieczki
```js
db.companies.updateOne(
  { "tours._id": tour1Id },
...
```
Dodanie nowego dnia w planie wycieczki
```js
db.companies.updateOne(
  { "tours._id": tour2Id },
  {
...
### 1. WADA: Problemy z aktualizacją zduplikowanych danych

Zmiana nazwy wycieczki wymaga aktualizacji w wielu miejscach
Krok 1: Aktualizacja w kolekcji firm
```js
db.companies.updateOne(
  { "tours._id": tour1Id },
...
Krok 2: Aktualizacja tytułu wycieczki we wszystkich rezerwacjach użytkowników
```js
db.users.updateMany(
  { "reservations.tourId": tour1Id },
...
Krok 3: Aktualizacja tytułu wycieczki we wszystkich recenzjach użytkowników
...
### 2. WADA: Złożone zapytania na zagnieżdżonych danych

Znalezienie wszystkich wycieczek z ceną poniżej 4000 PLN, które rozpoczynają się w czerwcu 2025 i mają tagi "kultura" oraz "zwiedzanie"
```js
